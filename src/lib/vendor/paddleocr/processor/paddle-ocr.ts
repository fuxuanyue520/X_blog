...
export class PaddleOcrService {
	...
	private resolveDetectionRuntimeOptions(
		...
		const { modelBuffer: _modelBuffer, ...instanceOptions } =
			this.options.detection ?? {};
	...
	public async recognize(
		...
		if (!this.detectionService || !this.recognitionService) {
			throw new Error(
				"PaddleOcrService is not initialized. Please call initialize() first.",
			);
		}
		...
		const detectionRuntimeOptions = this.resolveDetectionRuntimeOptions(
			options?.detection,
		);
		const recognitionRuntimeOptions = this.resolveRecognitionRuntimeOptions(
			options?.recognition,
		);
		const orderingOptions = this.resolveRecognitionOrderingOptions(
			options?.ordering,
		);
	...
	public async destroy(): Promise<void> {
		if (
			this.detectionSession &&
			typeof this.detectionSession.release === "function"
		) {
			await this.detectionSession.release();
		}
		if (
			this.recognitionSession &&
			typeof this.recognitionSession.release === "function"
		) {
